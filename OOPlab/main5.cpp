...
public:
    ...
    bool skill1(Champion& _champ){
        int damage = 100 + (this->physicalDamage * 20) / 100;
        return _champ.takePhysicalDamage(damage);
    }
    bool skill2(Champion& _champ){
        int damage = 20 + (this->physicalDamage * 80) / 100;
        return _champ.takePhysicalDamage(damage);
    }
};
...
public:
    ...
    bool skill1(Champion& _champ){
        int damage = 30 + (this->abilityPower * 70) / 100;
        return _champ.takeMagicDamage(damage);
    }
    bool skill2(Champion& _champ){
        int damage = 90 + (this->abilityPower * 30) / 100;
        return _champ.takeMagicDamage(damage);
    }
};
